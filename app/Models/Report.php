...
class Report
{
    ...
    public static function salesSummary(array $filters): array
    {
        ...
        // net_amount is what was charged at checkout and never changes
        // after the fact — a void/return is tracked separately in
        // refunded_amount, so revenue figures subtract it out here rather
        // than the sale record itself being mutated.
        $totals = (clone $query)->selectRaw('
                COUNT(*) as transaction_count,
                COALESCE(SUM(gross_amount), 0) as gross_amount,
                COALESCE(SUM(discount_amount), 0) as discount_amount,
                COALESCE(SUM(net_amount - refunded_amount), 0) as net_amount
            ')->first();
        ...
        $byPaymentMethod = (clone $query)
            ->select(
                ...
                DB::raw('SUM(net_amount - refunded_amount) as total')
            )
        ...
        $byDay = (clone $query)
            ->selectRaw('
                ...
                COUNT(*) as transaction_count,
                SUM(net_amount - refunded_amount) as net_amount
            ')
            ->groupBy('sale_date')
            ->orderBy('sale_date')
            ->get();
    ...
    public static function salesDetail(array $filters)
    {
        ...
        $query = DB::table('tbl_sales as s')
            ...
            ->select(
                ...
                's.discount_amount',
                's.refunded_amount',
                DB::raw('(s.net_amount - s.refunded_amount) as net_amount'),
                's.created_at',
    ...
    public static function salesDetailTotals(array $filters)
    {
        ...
        return $query->selectRaw('
            ...
            COALESCE(SUM(s.net_amount - s.refunded_amount), 0) as net_total
        ')->first();
    }
    ...
    public static function salesDetailByPaymentMethod(array $filters)
    {
        ...
        return $query->selectRaw('
            s.payment_method,
            COUNT(*) as count,
            COALESCE(SUM(s.net_amount - s.refunded_amount), 0) as total
        ')
    ...
    public static function topProducts(array $filters, int $limit = 20)
    {
        ...
        // Deduction happens per line here, not per sale: a partial void
        // only returns some of the units on one specific sale item, so
        // units_sold and revenue both need to subtract what that specific
        // line had returned/discounted, not the sale's overall refund.
        $query = DB::table('tbl_sales_items as si')
            ...
            ->select(
                ...
                'p.med_name',
                'p.brand_name',
                DB::raw('SUM(si.quantity_sold - si.returned_quantity) as units_sold'),
                DB::raw('SUM(si.total_price - si.discount_amount - si.refunded_amount) as revenue')
            )
    ...
    public static function salesByCashier(array $filters)
    {
        ...
        $query = DB::table('tbl_sales as s')
            ...
            ->select(
                ...
                DB::raw('SUM(s.net_amount - s.refunded_amount) as net_amount')
            )
            ->groupBy('u.id', 'u.name')
        ...
        public static function voidReturns(array $filters)
    {
        [$from, $to] = static::dateRange($filters);

        $query = DB::table('tbl_sale_return_items as ri')
            ->join('tbl_sale_returns as r', 'r.return_id', '=', 'ri.return_id')
            ->join('tbl_sales as s', 's.id', '=', 'r.sale_id')
            ->join('tbl_sales_items as si', 'si.id', '=', 'ri.sale_item_id')
            ->join('tbl_products as p', 'p.id', '=', 'si.product_id')
        ...
        static::applyBranch($query, $filters, 'r.branch_id');

        return $query->paginate($filters['per_page'] ?? 25)->withQueryString();
    }

    public static function voidReturnsTotals(array $filters)
    {
        [$from, $to] = static::dateRange($filters);
        ...
        return $query->selectRaw('
            ...
            COALESCE(SUM(ri.quantity_returned), 0) as items_returned,
            COALESCE(SUM(ri.refund_amount), 0) as total_refunded
        ')->first();
    }
