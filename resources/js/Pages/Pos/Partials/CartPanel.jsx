...
export default function CartPanel({
    ...
    // A discount % selected with nothing checked to apply it to means the
    // cashier picked PWD/Senior or Single Mother but forgot to mark which
    // items qualify — checking out here would silently charge full price
    // while still showing the discount button as "active".
    const hasUncheckedDiscount =
        discountPercent > 0 && !cartItems.some((item) => item.applyDiscount);
    const checkoutDisabled = syncing || hasUncheckedDiscount;
        ...
        <Card className="flex h-full w-full max-w-full flex-col overflow-hidden">
            ...
            <CardContent className="flex min-w-0 flex-1 flex-col gap-4 overflow-hidden pt-4">
                <CartTable
                    ...
                    onToggleDiscount={onToggleItemDiscount}
                    onToggleVatExempt={onToggleVatExempt}
                    onRemove={onRemove}
                    ...
                    <>
                        ...
                        <div className="mt-auto space-y-3 rounded-lg bg-muted/50 p-4">
                            ...
                            {discountPercent > 0 && (
                                <p
                                    className={
                                        hasUncheckedDiscount
                                            ? "text-xs font-medium text-red-600"
                                            : "text-xs text-muted-foreground"
                                    }
                                >
                                    {hasUncheckedDiscount
                                        ? "Check at least one item above to apply this discount before checking out."
                                        : "Check items in the cart above to apply this discount to them."}
                                </p>
                            )}
