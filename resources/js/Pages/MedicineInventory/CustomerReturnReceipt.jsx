...
function receiptNumber(returnId) {
    return String(returnId ?? "").padStart(6, "0");
}
 ...
 * Standalone printable receipt for medicine returned by a customer.
 * Accessed via GET /customer-returns/{id}/receipt
 */
export default function CustomerReturnReceipt({ customerReturn }) {
    const items = customerReturn.items ?? [];

    const customerName = customerReturn.customer
        ? `${customerReturn.customer.first_name ?? ""} ${
              customerReturn.customer.last_name ?? ""
          }`.trim()
        : "";
    ...
    return (
        <>
            <Head
                title={`Return Receipt — #${receiptNumber(customerReturn.return_id)}`}
            />
            ...
            >
                ...
                <div className="flex items-center gap-4">
                    <span className="text-sm text-gray-400 font-mono">
                        #{receiptNumber(customerReturn.return_id)}
                    </span>
                    <button
                ...
                >
                    ...
                    <div className="border border-gray-900 mb-2">
                        <div className="grid grid-cols-[1fr_140px] border-b border-gray-900">
                            <HeaderCell label="Received from">
                                {customerName}
                            </HeaderCell>
                        ...
                        <div className="grid grid-cols-[1fr_140px] border-b border-gray-900">
                            <HeaderCell label="Customer Type">
                                {customerReturn.customer?.customer_type}
                            </HeaderCell>
                            <HeaderCell label="Branch" borderedLeft>
                                {customerReturn.branch?.branch_name}
                            </HeaderCell>
                        </div>
                        <div className="grid grid-cols-[1fr_140px]">
                            ...
                            <HeaderCell label="Return #" borderedLeft blank />
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 mb-2 text-[10px]">
                        <FieldLine label="Customer" value={customerName} />
                        <FieldLine
                            label="Branch"
                            value={customerReturn.branch?.branch_name}
                        />
                        <FieldLine
                            label="Received by"
                            value={customerReturn.received_by}
                        />
                        <FieldLine
                            label="Recorded"
                            value={formatDateTime(customerReturn.created_at)}
                        />
                    </div>
                    ...
                    {customerReturn.remarks && (
                        <div className="border border-gray-400 px-2 py-1 mb-2 text-[10px]">
                            ...
                            <span>{customerReturn.remarks}</span>
                        </div>
                    )}
                    ...
                    <div className="mt-6 grid grid-cols-[1fr_200px] gap-4 items-end">
                        <div className="grid grid-cols-3 gap-3">
                            <SignatureBox
                                label="Received By"
                                name={customerReturn.received_by}
                            />
                            <SignatureBox label="Inspected By" blank />
                            <SignatureBox label="Approved By" blank />
                        </div>
                        <div className="text-[9px]">
                            <p className="italic mb-6 leading-snug">
                                Returned the above merchandise, received in good
                                order and condition
                            </p>
                            <p>
                    ...
                    <p className="text-[8px] text-gray-400 text-center mt-2">
                        System-generated · Return Receipt #
                        {receiptNumber(customerReturn.return_id)} · Westpoint
